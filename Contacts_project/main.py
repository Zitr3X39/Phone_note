...
class Main(tk.Frame):
    ...
    def update_record_dialog(self, name, tel, email):
    ...
    def delete_records(self):
        for selections_item in self.tree.selection():
            self.db.c.execute('''
        DELETE FROM db WHERE id = ?''', 
        (self.tree.set(selections_item, '#1'),))
        self.db.conn.commit()
        self.view_records()
...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        self.button_canel = tk.Button(self, text='Закрыть',
                                      command=self.destroy)
        self.button_canel.place(x=300, y=170)
...
class Update(Child):
    ...
    def init_edit(self):
        ...
        btn_edit.bind('<Button-1>', lambda event:
                      self.view.update_record_dialog(self.entry_name.get(),
                                              self.entry_email.get(),
                                              self.entry_tel.get()))
        btn_edit.bind('<Button-1>', lambda event:
                      self.destroy(), add='+')
